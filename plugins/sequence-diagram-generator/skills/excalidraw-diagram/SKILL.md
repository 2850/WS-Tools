...
description: Generate Excalidraw diagrams from text content for Obsidian. Use when user asks to create diagrams, flowcharts, mind maps, sequence diagrams, or visual representations in Excalidraw format. Triggers on "Excalidraw", "画图", "流程图", "思维导图", "时序图", "可视化", "diagram".
metadata:
  version: 1.2.0
---
...
3. **Generate the file using Python script** (preferred) OR generate JSON directly
4. Verify the output file is readable in Obsidian
5. Notify user with file path and confirm save successful

## ⚠️ Critical: File Generation Method

**Always use the Python script** `scripts/generate_excalidraw.py` for complex diagrams
(especially sequence diagrams). Direct JSON-in-string generation causes two common bugs:

| Bug | Cause | Fix in script |
|-----|-------|---------------|
| `\\json` instead of ` ```json ` | Backtick escape in Python strings | `chr(96)*3` trick |
| `"x2"/"y2" not found` | Wrong arrow/line format | `points` array helpers |
| Text embedded in rectangle | `rectangle.text` doesn't exist | Separate `text` element with `containerId` |

### Using the script

1. Read `scripts/generate_excalidraw.py` into context
2. Copy/adapt the element-builder pattern to match your diagram
3. Run via PowerShell temp-file pattern to avoid heredoc issues:

```powershell
$py = @'
import sys
sys.path.insert(0, r"D:\path\to\skills\excalidraw-diagram\scripts")
from generate_excalidraw import make_text, make_rect, make_arrow, make_line, write_excalidraw_md

elements = []
# ... build your elements ...
write_excalidraw_md(elements, r"D:\path\to\output.excalidraw.md")
'@
$py | Out-File "$env:TEMP\build_diagram.py" -Encoding UTF8
python "$env:TEMP\build_diagram.py"
```
...
| **时序图** | Sequence | 多角色之间的互动顺序、通信协议 | Actor 顶部方框 + 垂直虚线生命線 + 横向箭头 |
...
## JSON Element Rules (Critical)

### ❌ Common Mistakes

```json
// WRONG: arrow/line with x2, y2
{ "type": "arrow", "x": 100, "y": 100, "x2": 300, "y2": 100 }

// WRONG: text embedded inside rectangle
{ "type": "rectangle", "text": "My Label" }
```

### ✅ Correct Patterns

```json
// Arrow with points array
{ "type": "arrow", "x": 100, "y": 100, "points": [[0,0],[200,0]], "endArrowhead": "arrow" }

// Rectangle + separate text bound via containerId
{ "type": "rectangle", "id": "r1", "boundElements": [{"type":"text","id":"t1"}] }
{ "type": "text", "id": "t1", "containerId": "r1", "text": "My Label" }
```
